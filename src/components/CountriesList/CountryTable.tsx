...
export default function CountriesTable({
  ...
  useEffect(() => {
    ...
    countries.forEach((name) => {
      ...
      const changed = fieldsToCheck.some(
        (field) =>
          yearData[field as keyof YearlyData] !==
          prevData?.[field as keyof YearlyData],
      );
  ...
  const sortedCountries = useMemo(() => {
    if (!data) return [];
    return [...countries].sort((aName, bName) => {
  ...
  }, [countries, data, selectedYear, sortBy, sortOrder]);
    ...
    >
      <thead>
        <tr>
          <th onClick={() => handleSort("name")}>
            <button>Sort by country name</button>
          </th>
      ...
      <tbody>
        {sortedCountries.map((name) => {
          ...
          const highlightClass = highlightRows.includes(name)
            ? "highlight"
            : "";
          return (
